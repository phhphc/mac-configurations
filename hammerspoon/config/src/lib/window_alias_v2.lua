...
---@param win hs.window
---@return string
function M:getAlias(win)
    ---@type number | nil
    local id = win:id()
    return (id and aliases[id]) or win:title()
end

---@param win hs.window
function M:setAlias(win)
    ---@type number | nil
    local id = win:id()
...
function backupAliases()
    ...
    print("inspected alias: " .. hs.inspect(aliases))
...
function restoreAliases()
    ...
    for idStr, alias in pairs(cleanAliases) do
        ...
        if id and type(alias) == "string" then aliases[id] = alias end
    end

    print("restored aliases: " .. hs.inspect(aliases))
end
